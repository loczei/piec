...
void start() {
  ...
  printOnDisplay("Device ready", "Prosze czekac");
  ...
  printOnDisplay("Heating", "Prosze czekac");
...
void cj125Init() {
    printOnDisplay("cj125 init", "Prosze czekac");
  ...
  printOnDisplay("Device reset", "Prosze czekac");
  digitalWrite(LED_STATUS_POWER, HIGH);
  digitalWrite(LED_STATUS_HEATER, HIGH);
  delay(200);
  digitalWrite(LED_STATUS_POWER, LOW);
...
void cj125Update() {
  ...
  lambda = Lookup_Lambda(adcValue_UA);
      
  //Calculate Oxygen Content.
  oxygen = Lookup_Oxygen(adcValue_UA);
